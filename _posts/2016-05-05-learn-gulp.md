...
```js
{
  ...
  "license": "ISC",    //项目许可协议
  "devDependencies": {    //项目依赖的插件，如果 node_modules 被删除了，npm install 就可以下载项目所有依赖的模块
    "gulp": "^3.8.11",
    "gulp-less": "^3.0.0"
  }
}
...
```html 
cnpm install --save-dev（将其添加到项目开发的依赖 package.json 目录下的 devDependencies）
```
...
要使用 Gulp 去处理文件的话，需要在项目下面去创建一个名为 `gulpfile.js` 的文件。

`gulpfile.js`是gulp项目的配置文件，是位于项目`根目录的普通js文件`（其实将gulpfile.js放入其他文件夹下亦可）。

在这个文件里面，我们可以去添加要执行的任务。
...
//添加一个任务
gulp.task('任务的名字', function () {
    gulp.src('src/less/index.less') //该任务针对的文件
        .pipe(less()) //该任务调用的模块
        .pipe(gulp.dest('src/css')); //将会在src/css下生成index.css
});
 
// 第一个参数，名字为 default 表示是一个默认的任务
// 第二个参数是一个要执行任务的列表
